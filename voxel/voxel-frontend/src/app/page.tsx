  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      for (let line = 0; line < 3; line++) {
        ctx.beginPath()
        const amp   = [18, 10, 6][line]
        const freq  = [0.018, 0.025, 0.012][line]
        const speed = [0.03, 0.05, 0.02][line]
        const alpha = [0.35, 0.2, 0.12][line]
        const yOff  = h * 0.5 + (line - 1) * 28

        ctx.moveTo(0, yOff)
        for (let x = 0; x <= w; x += 2) {
          const y = yOff +
            Math.sin(x * freq + frame * speed) * amp +
            Math.sin(x * freq * 1.7 + frame * speed * 1.3) * (amp * 0.5)
          ctx.lineTo(x, y)
        }
        ctx.strokeStyle = `rgba(20, 184, 166, ${alpha})`
        ctx.lineWidth   = [2, 1.5, 1][line]
        ctx.stroke()
      }
      frame++
    ...
    <main className="min-h-screen w-full flex flex-col relative overflow-hidden"
      ...
      <section className="relative z-10 flex-1 flex flex-col items-center justify-center text-center px-6 py-10">
        ...
        <div className="relative mb-12 flex items-center justify-center">
          ...
          {/* Orb — dark circle with teal glow, matching Visily */}
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 mb-16">
          <Link href="/register"
                ...
                style={{
                  ...
                  boxShadow: '0 8px 32px rgba(11,148,136,0.5)',
                  minWidth: '220px',
                }}>
            Get Started Free
            <ArrowRight size={18} />
        ...
        <div className="relative w-full max-w-lg h-20 mb-16">
          ...
          <p className="absolute -bottom-5 left-1/2 -translate-x-1/2 text-xs font-dm"
             style={{ color: 'var(--muted)' }}>Live speech processing</p>
        </div>

        {/* Stats */}
        <div className="flex flex-wrap items-center justify-center gap-8 sm:gap-16">
          {[
            { value: '98%', label: 'ASR Accuracy' },
            ...
            <div key={label} className="flex items-center gap-8 sm:gap-16">
              <div className="text-center">
                ...
                <p className="text-xs font-dm mt-1" style={{ color: 'var(--muted)' }}>{label}</p>
              </div>
              {i < arr.length - 1 && (

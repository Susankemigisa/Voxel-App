  ...
  return (
    <ScrollView contentContainerStyle={s.container} keyboardShouldPersistTaps="handled">
      ...
      <View style={s.card}>
        ...
        {/* Tabs */}
        <View style={s.tabs}>
          <TouchableOpacity style={s.tabInactive} onPress={() => router.replace('/(auth)/register')}>
            <Text style={s.tabInactiveText}>Sign Up</Text>
          </TouchableOpacity>
          <View style={s.tabActive}>
            <Text style={s.tabActiveText}>Sign In</Text>
          </View>
        </View>
        ...
        <TouchableOpacity onPress={() => router.replace('/(auth)/register')} style={s.switchWrap}>
          <Text style={s.switchText}>Don't have an account? <Text style={{ color: COLORS.teal2 }}>Create one free</Text></Text>
        </TouchableOpacity>
      </View>
    </ScrollView>
  )
...
const s = StyleSheet.create({
  ...
  input:        { backgroundColor: COLORS.surface, borderWidth: 1, borderColor: COLORS.border, borderRadius: RADIUS.lg, paddingHorizontal: 14, paddingVertical: 12, color: COLORS.text, fontSize: 14, marginBottom: 0 },
